...
The committed `.env` contains only the browser-safe Supabase URL and publishable anon key as a build fallback. The same public defaults are mirrored in `src/config/publicEnvFallback.js` so Vercel Functions still have a safe fallback if a deployment excludes `.env`. Values configured in Vercel have higher priority and replace these defaults during the build and Function runtime. Never place a service-role key, Gemini key, or another private credential in either fallback location.

The production build is deliberately flattened: `dist` contains `index.html` and its generated `.css`/`.js` files directly, without a nested `dist/assets` directory. The source remains component-based React for maintainability; Vite converts it into ordinary browser HTML, CSS, and JavaScript for deployment.
